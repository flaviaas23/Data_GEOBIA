...
if SAVE_TO_PARQUET:
    ...
    df_toPCA, max_bt, min_bt = gen_dfToPCA_filter(dates, band_tile_img_files, dask=1, n_part=63, ar_pos=0, sh_print=1)
    #band_tile_img_files
    t1=time.time()
    ...
    save_path = cur_dir + '/data/parquet_npart_63/'
    obj_dic = {}
    obj_dic = {
        "max_bt": max_bt, 
        "min_bt": min_bt,
    ...
    file_to_save = save_path+name_img+'_max_min_bt_npart_63.pkl'
    ...
    f_name = name_img+'_df_toPCA_full_npart_63'
    print (f'saving df to {save_path+f_name}') if sh_print_n2 else None
    t1-time.time()
    df_toPCA.to_parquet(save_path+f_name, engine='pyarrow',write_metadata_file=False)
    t2=time.time()
...
if 0: ######testes com tamanho de particoes 
    ...
    print ("df_toPCA_coords cols: {df_toPCA_coords.columns}")
    del df_toPCA_coords, df_toPCA
    quit()
    t1=time.time()
    ...
    t1=time.time()
    npart=df_toPCA_loc.npartitions
    ...
    pca_components_df_merge= df_toPCA_coords.merge(pca_components_df, how='left', on='orig_index')#.reset_index() #on= ['index_orig'],
    ...
    for c in comps_sel:
        ...
        img_comp_dic_df[c] = pca_components_df_merge.pivot_table(index='coords_0', columns='coords_1', values=c)
        img_comp_dic[c] = img_comp_dic_df[c].to_dask_array(lengths=True) 
    t4=time.time()
    ...
    print (f'tempo para pivot table of TO_DASK_ARRAY all comps {comps_sel} COM repartition: {t4-t3}')
    ...
    print (f'{c} chunksize: {img_comp_dic[c].chunksize}\nchunks: {img_comp_dic[c].chunks}')
    #ZARR
    ...
    del df_toPCA, df_toPCA_loc, df_toPCA_coords#, img_comp_dic_df, df_toPCA_head
...
if do_PCA:
    ...
    dX = df_toPCA[cols_to_pca].to_dask_array(lengths=True)
#%%
test1=False
if test1:
    t1=time.time()
...
if func_random == 2:
    ...
    print ('Explicabilidade: {random_pca.explained_variance_ratio_}')
    # Gets the principal components of pca
    ...
    pca_components_df = dd.from_dask_array(principalComponents, columns=cols)
    t3=time.time()
...
del pca_components_df2, df_toPCA
gc.collect()
...
t3=time.time()
print (f'tempo divisions = {t2-t1},\t tempo npartitions={t3-t2}')
...
t1=time.time()
#o pca_components_df tem erro para ver o index e talvez por isso  o merge tb 
...
for c in [c1]:#comps_sel:
    tp1=time.time()
    #img_comp_dic[c] = pca_components_df_merge.pivot_table(index='coords_0', columns='coords_1', values=c).values
    img_comp_df= pca_components_df_merge.pivot_table(index='coords_0', columns='coords_1', values=c)
    ...
    tp2=time.time()
    print (f'tempo para pivot de {c}: {tp2-tp1}')
...
if SAVE_IMG_PCA:
    for c in [c1]:#comps_sel:
        ...
        t1=time.time()
        img_comp_dic[c].compute_chunk_sizes() 
        #img_comp_v.compute_chunk_sizes() 
        ...
        #img_comp_v.to_zarr(save_path+f_name)
        t2=time.time()
